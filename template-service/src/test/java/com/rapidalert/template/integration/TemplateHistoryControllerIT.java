...
public class TemplateHistoryControllerIT extends ITBase {
    ...
    @Test
    @Disabled("Pre-existing brittle test: passes templateId to GET /history/{id} which "
            + "the service treats as a historyId. Only works when both auto-IDs collide. "
            + "Tracked separately — needs refactor to capture the returned historyId.")
