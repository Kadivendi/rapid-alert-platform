...
@Table(
        ...
        uniqueConstraints = {
                ...
                // The same FCM registration token must not be tied to two
                // recipients of the same client — Firebase considers tokens
                // device-unique, and double-registration would have us push
                // the same alert twice.
                @UniqueConstraint(name = "recipients_unq_clientId-fcmToken", columnNames = {"clientId", "fcmToken"})
        },
        indexes = {
...
public class Recipient implements BaseEntity<Long> {
    ...
    /**
     * Firebase Cloud Messaging registration token captured from the
     * recipient's mobile device. Used by sender-service's
     * {@code FcmAlertService} to dispatch push notifications.
     */
    @Column(length = 4096)
